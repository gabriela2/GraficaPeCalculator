...
void init (void)  // initializare fereastra de vizualizare
{
	...
	gluOrtho2D (-500.0, 700.0, -600.0, 700.0); // sunt indicate coordonatele extreme ale ferestrei de vizualizare
...
void mouse(int button, int state, int x, int y)
{
    ...
    case GLUT_LEFT_BUTTON:
        if (state == GLUT_DOWN)
            alpha = -1.5; glutIdleFunc(miscas);
        break;
    case GLUT_RIGHT_BUTTON:
        if (state == GLUT_DOWN)
            alpha = 1.5; glutIdleFunc(miscad);
        break;
    default:
        break;
    }
